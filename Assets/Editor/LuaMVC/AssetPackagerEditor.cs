...
namespace LuaMVC.Editor
{
    ...
    public class AssetPackagerEditor : EditorWindow
    {
        ...
        private static string luaScriptPath = Application.streamingAssetsPath + "/Lua/"; // todo 考虑更改
        private static string luaScriptTargetPath = Application.streamingAssetsPath + "/Data/";
        ...
        private static void PackPrefab(BuildTarget buildTarget)
        {
            ...
            //AssetDatabase.Refresh();
            if (!Directory.Exists(luaScriptTargetPath))Directory.CreateDirectory(luaScriptTargetPath);
            if (Directory.Exists(prefabsTargetPath))
            {
                // 直接拷贝assets文件到目标文件夹
                RecursionCopyAssetsbundleScript(prefabsTargetPath);
                WriteMD5(luaScriptTargetPath);
                AssetDatabase.Refresh();
            }
            Debug.Log("打包Assetbundle完毕");
        }


        private static void RecursionCopyAssetsbundleScript(string assetsPath)
        {
            string[] filesPath = Directory.GetFiles(assetsPath);
            for (int i = 0; i < filesPath.Length; i++)
            {
                FileInfo fileInfo = new FileInfo(filesPath[i]);
                if ((fileInfo.Name.Contains(".unity3d") || fileInfo.Name.Contains(".manifest")) || fileInfo.Name == "Assetbundle" && !fileInfo.Name.Contains(".meta"))
                {
                    File.Copy(fileInfo.FullName, luaScriptTargetPath + fileInfo.Name, true);
                }
            }
            string[] childrenPath = Directory.GetDirectories(assetsPath);
            for (int i = 0; i < childrenPath.Length; i++)
                RecursionCopyAssetsbundleScript(childrenPath[i]);
        }
        ...
        {
            using (StreamWriter write = File.CreateText(Application.dataPath + "/StreamingAssets/md5list.txt"))
            {
        ...
        private static void RecursionWriteMD5(string dirPath, StreamWriter write)
        {
            ...
            for (int i = 0; i < filesPath.Length; i++)
            {
                FileInfo fileInfo = new FileInfo(filesPath[i]);
                //打包 ab 包的做法
                if (fileInfo.Extension.Contains(".unity3d") || fileInfo.Extension.Contains(".manifest") || fileInfo.Name == "Assetbundle" || fileInfo.Extension.Equals("") && !fileInfo.Name.Contains(".meta"))
                {
